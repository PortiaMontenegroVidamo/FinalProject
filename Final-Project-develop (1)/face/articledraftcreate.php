<?php
        if (isset($_POST['submit'])){
            ...
            $connection = mysqli_connect("localhost", "root", "", "pmdb");
                ...
                $query = "INSERT INTO articledraft (title,author,date,content,image,category,status) VALUES ('$title', '$author', $'date', '$content', '$image', '$category', '$status')";
...
<body>
<form method="post" action="articledraftphp.html">
    ...
    <form class="create-article" method="POST">
        <div class="article-editor text bg-primary mb-3">
            ...
            <div class="card-body">
                ...
                <textarea class="text-primary" id="editor" name="article_content" placeholder="Some quick example text to build on the card title and make up the bulk of the card's content."></textarea>
                <hr class="my-4">
                <button type="button" class="btn btn-outline-success">Publish now</button>
                <button type="button" name="submit" value="create" class="btn btn-outline-info">Save to Draft</button>
